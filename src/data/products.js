export const PRODUCTS = [
  // Suits & Blazers
  {
    id: 1,
    name: "Dhaka Midnight Suit",
    category: "Suits & Blazers",
    price: 45000,
    rating: 4.9,
    reviews: 87,
    badge: "Bestseller",
    image:
      "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=600&q=80",
    description:
      "A masterfully tailored two-piece suit in deep midnight navy. Crafted from premium Bengali wool-blend fabric with a slim silhouette, notch lapel, and hand-finished buttonholes. The definitive suit for the modern Bangladeshi gentleman.",
    tags: ["suit", "formal", "navy", "slim-fit"],
  },
  {
    id: 2,
    name: "Bengal Charcoal Blazer",
    category: "Suits & Blazers",
    price: 28000,
    rating: 4.8,
    reviews: 64,
    badge: "New",
    image:
      "https://images.unsplash.com/photo-1552374196-1ab2a1c593e8?w=600&q=80",
    description:
      "A refined single-breasted blazer in charcoal grey. Constructed from a fine wool-polyester blend for year-round comfort. Features a structured shoulder, double-vent back, and contrast silk lining.",
    tags: ["blazer", "charcoal", "formal", "wool"],
  },
  {
    id: 3,
    name: "Ruposhi Double-Breasted Suit",
    category: "Suits & Blazers",
    price: 52000,
    rating: 4.7,
    reviews: 43,
    badge: "Limited",
    image:
      "https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?w=600&q=80",
    description:
      "An assertive double-breasted suit in warm camel. A bold statement for the man who commands attention. Peak lapels, six-button front, and a clean wide-leg trouser complete this power look.",
    tags: ["suit", "double-breasted", "camel", "statement"],
  },
  {
    id: 4,
    name: "Sylhet Linen Blazer",
    category: "Suits & Blazers",
    price: 22000,
    rating: 4.6,
    reviews: 51,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1593030761757-71fae45fa0e7?w=600&q=80",
    description:
      "A lightweight linen blazer in natural ecru — perfect for the Bangladeshi climate. Unstructured construction for effortless drape, with patch pockets and a relaxed fit. Wear it over a kurta or a crisp white shirt.",
    tags: ["blazer", "linen", "summer", "casual"],
  },
  {
    id: 5,
    name: "Padma Three-Piece Suit",
    category: "Suits & Blazers",
    price: 65000,
    rating: 5.0,
    reviews: 29,
    badge: "Editor's Pick",
    image:
      "https://images.unsplash.com/photo-1560250097-0b93528c311a?w=600&q=80",
    description:
      "The pinnacle of Bangladeshi tailoring. A three-piece suit in rich burgundy with a matching waistcoat. Hand-stitched lapels, mother-of-pearl buttons, and a full silk lining. Made to measure on request.",
    tags: ["suit", "three-piece", "burgundy", "luxury"],
  },
  {
    id: 6,
    name: "Chittagong Check Blazer",
    category: "Suits & Blazers",
    price: 31000,
    rating: 4.7,
    reviews: 38,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1617127365659-c47fa864d8bc?w=600&q=80",
    description:
      "A classic Glen plaid check blazer in navy and cream. Slim fit with a single vent and flap pockets. Pairs equally well with tailored trousers or dark denim.",
    tags: ["blazer", "check", "plaid", "smart-casual"],
  },

  // Shirts
  {
    id: 7,
    name: "Muslin White Dress Shirt",
    category: "Shirts",
    price: 8500,
    rating: 4.8,
    reviews: 142,
    badge: "Bestseller",
    image:
      "https://images.unsplash.com/photo-1598033129183-c4f50c736f10?w=600&q=80",
    description:
      "Crafted from the finest Dhaka muslin — the legendary fabric that made Bengal famous. An impeccably white dress shirt with a spread collar, French cuffs, and mother-of-pearl buttons. A wardrobe essential.",
    tags: ["shirt", "muslin", "white", "formal"],
  },
  {
    id: 8,
    name: "Bengal Blue Oxford Shirt",
    category: "Shirts",
    price: 7200,
    rating: 4.6,
    reviews: 98,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1620012253295-c15cc3e65df4?w=600&q=80",
    description:
      "A classic Oxford shirt in rich Bengal blue. Slightly relaxed fit with a button-down collar and a single chest pocket. Made from a fine cotton-linen blend for all-day comfort.",
    tags: ["shirt", "oxford", "blue", "casual"],
  },
  {
    id: 9,
    name: "Rajshahi Silk Evening Shirt",
    category: "Shirts",
    price: 14500,
    rating: 4.9,
    reviews: 56,
    badge: "New",
    image:
      "https://images.unsplash.com/photo-1603252109303-2751441dd157?w=600&q=80",
    description:
      "A luxurious evening shirt in pure Rajshahi silk. The fabric has a natural lustre that catches the light beautifully. French cuffs, a pleated bib front, and a hidden placket make this the definitive formal shirt.",
    tags: ["shirt", "silk", "evening", "formal"],
  },
  {
    id: 10,
    name: "Striped Cotton Casual Shirt",
    category: "Shirts",
    price: 5800,
    rating: 4.5,
    reviews: 77,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1588359348347-9bc6cbbb689e?w=600&q=80",
    description:
      "A relaxed fit casual shirt in fine cotton with subtle vertical stripes in navy and white. Point collar, single-button cuffs, and a curved hem. The perfect weekend shirt.",
    tags: ["shirt", "striped", "casual", "cotton"],
  },
  {
    id: 11,
    name: "Ivory Linen Kurta Shirt",
    category: "Shirts",
    price: 9800,
    rating: 4.7,
    reviews: 89,
    badge: "Popular",
    image:
      "https://images.unsplash.com/photo-1610652492500-ded49ceeb378?w=600&q=80",
    description:
      "A contemporary luxury kurta shirt in ivory linen. Mandarin collar, subtle pintuck detailing on the placket, and a relaxed silhouette. The perfect bridge between traditional and modern dressing.",
    tags: ["kurta", "linen", "ivory", "fusion"],
  },
  {
    id: 12,
    name: "Midnight Black Tuxedo Shirt",
    category: "Shirts",
    price: 12000,
    rating: 4.8,
    reviews: 44,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1507679799987-c73779587ccf?w=600&q=80",
    description:
      "A striking black tuxedo shirt in fine Egyptian cotton. Pleated front, spread collar, and polished onyx cufflinks included. For the man who redefines black-tie.",
    tags: ["shirt", "tuxedo", "black", "formal"],
  },

  // Trousers & Chinos
  {
    id: 13,
    name: "Charcoal Wool Dress Trousers",
    category: "Trousers & Chinos",
    price: 16500,
    rating: 4.8,
    reviews: 73,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1473966968600-fa801b869a1a?w=600&q=80",
    description:
      "Impeccably cut dress trousers in charcoal wool. Flat front, slim leg, and a mid-rise waist. Hand-finished side seams and a full lining for the finest drape. The cornerstone of a gentleman's wardrobe.",
    tags: ["trousers", "wool", "charcoal", "formal"],
  },
  {
    id: 14,
    name: "Khaki Bengal Chinos",
    category: "Trousers & Chinos",
    price: 9500,
    rating: 4.6,
    reviews: 108,
    badge: "Bestseller",
    image:
      "https://images.unsplash.com/photo-1624378439575-d8705ad7ae80?w=600&q=80",
    description:
      "The perfect chino in warm khaki. A slim straight cut that works from the office to an evening out. Crafted from a premium cotton twill with a subtle sheen and four-pocket design.",
    tags: ["chinos", "khaki", "casual", "smart"],
  },
  {
    id: 15,
    name: "Navy Pinstripe Trousers",
    category: "Trousers & Chinos",
    price: 18000,
    rating: 4.7,
    reviews: 47,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1542272604-787c3835535d?w=600&q=80",
    description:
      "Classic pinstripe trousers in deep navy. A wide leg and high-rise cut give an elegant, vintage-inspired silhouette. Pairs beautifully with the Dhaka Midnight Suit jacket or worn separately.",
    tags: ["trousers", "pinstripe", "navy", "formal"],
  },
  {
    id: 16,
    name: "Olive Slim Chinos",
    category: "Trousers & Chinos",
    price: 8800,
    rating: 4.5,
    reviews: 92,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1519238263530-99bdd11df2ea?w=600&q=80",
    description:
      "Versatile slim chinos in muted olive. A cotton-stretch blend for comfort and ease of movement. Suitable for smart-casual occasions — pair with a blazer or a linen shirt.",
    tags: ["chinos", "olive", "slim", "casual"],
  },
  {
    id: 17,
    name: "Cream Linen Trousers",
    category: "Trousers & Chinos",
    price: 12000,
    rating: 4.7,
    reviews: 61,
    badge: "New",
    image:
      "https://images.unsplash.com/photo-1598033129183-c4f50c736f10?w=600&q=80",
    description:
      "Relaxed wide-leg linen trousers in warm cream. Ideal for the Bangladeshi heat. An elasticated waistband with a drawstring and deep side pockets make these as comfortable as they are stylish.",
    tags: ["trousers", "linen", "cream", "summer"],
  },

  // Outerwear & Coats
  {
    id: 18,
    name: "Dhaka Wool Overcoat",
    category: "Outerwear & Coats",
    price: 48000,
    rating: 4.9,
    reviews: 39,
    badge: "Limited",
    image:
      "https://images.unsplash.com/photo-1539533018447-63fcce2678e3?w=600&q=80",
    description:
      "A sweeping double-breasted overcoat in charcoal wool. Structured shoulders, a belted back, and deep pockets. For the cool Dhaka winter evenings when only the finest will do.",
    tags: ["overcoat", "wool", "charcoal", "winter"],
  },
  {
    id: 19,
    name: "Bengal Camel Trench Coat",
    category: "Outerwear & Coats",
    price: 42000,
    rating: 4.8,
    reviews: 55,
    badge: "Bestseller",
    image:
      "https://images.unsplash.com/photo-1551488831-00ddcb6c6bd3?w=600&q=80",
    description:
      "The quintessential trench coat in warm camel. Water-resistant gabardine, storm flap, gun flap, and a belted waist. A perennial investment piece that anchors any wardrobe.",
    tags: ["trench", "camel", "classic", "outerwear"],
  },
  {
    id: 20,
    name: "Padma Leather Bomber",
    category: "Outerwear & Coats",
    price: 38000,
    rating: 4.7,
    reviews: 42,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1520975954732-35dd22299614?w=600&q=80",
    description:
      "A luxurious bomber jacket in full-grain dark brown leather. Ribbed collar, cuffs, and hem. Quilted lining for warmth. A modern classic that only gets better with age.",
    tags: ["bomber", "leather", "brown", "casual"],
  },
  {
    id: 21,
    name: "Sylhet Quilted Jacket",
    category: "Outerwear & Coats",
    price: 22000,
    rating: 4.6,
    reviews: 67,
    badge: "New",
    image:
      "https://images.unsplash.com/photo-1544022613-e87ca75a784a?w=600&q=80",
    description:
      "A lightweight quilted jacket in midnight navy. Packable and versatile — ideal for transitional weather. Stand collar, two-way zip, and interior phone pocket.",
    tags: ["jacket", "quilted", "navy", "lightweight"],
  },

  // Footwear
  {
    id: 22,
    name: "Dhaka Oxford Shoes",
    category: "Footwear",
    price: 18500,
    rating: 4.9,
    reviews: 94,
    badge: "Bestseller",
    image:
      "https://images.unsplash.com/photo-1614252235316-8c857d38b5f4?w=600&q=80",
    description:
      "Handcrafted full-grain leather Oxford shoes in classic black. Goodyear welted construction for durability and resolability. A leather sole with rubber heel tap. The definitive dress shoe.",
    tags: ["oxford", "leather", "black", "formal"],
  },
  {
    id: 23,
    name: "Bengal Derby Shoes",
    category: "Footwear",
    price: 16000,
    rating: 4.7,
    reviews: 71,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=600&q=80",
    description:
      "Open-laced Derby shoes in rich tan leather. A more casual alternative to the Oxford, suitable for both office and weekend. Blake-stitched construction with a leather-lined interior.",
    tags: ["derby", "tan", "leather", "smart-casual"],
  },
  {
    id: 24,
    name: "Rajshahi Loafers",
    category: "Footwear",
    price: 14500,
    rating: 4.8,
    reviews: 83,
    badge: "Popular",
    image:
      "https://images.unsplash.com/photo-1533867617858-e7b97e060509?w=600&q=80",
    description:
      "Slip-on penny loafers in burnished cognac leather. Leather sole, leather lining, and a classic penny strap. Effortlessly elegant — wear them with or without socks.",
    tags: ["loafers", "cognac", "leather", "casual"],
  },
  {
    id: 25,
    name: "Chittagong Chelsea Boots",
    category: "Footwear",
    price: 22000,
    rating: 4.8,
    reviews: 59,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1543163521-1bf539c55dd2?w=600&q=80",
    description:
      "Sleek Chelsea boots in deep noir leather. Elastic side panels, a pull tab, and a block heel. Resolable leather sole. The most versatile boot in a gentleman's collection.",
    tags: ["boots", "chelsea", "black", "leather"],
  },
  {
    id: 26,
    name: "Muslin White Sneakers",
    category: "Footwear",
    price: 12000,
    rating: 4.6,
    reviews: 112,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1525966222134-fcfa99b8ae77?w=600&q=80",
    description:
      "Minimal leather sneakers in clean white. A slim profile with a gum sole and tonal stitching. The refined sneaker that elevates any casual outfit.",
    tags: ["sneakers", "white", "leather", "minimal"],
  },
  {
    id: 27,
    name: "Bengal Suede Monk Strap",
    category: "Footwear",
    price: 19500,
    rating: 4.7,
    reviews: 48,
    badge: "New",
    image:
      "https://images.unsplash.com/photo-1491553895911-0055eca6402d?w=600&q=80",
    description:
      "A distinguished double monk strap shoe in rich burgundy suede. Polished brass buckles and a sleek pointed toe. For the man who makes an entrance.",
    tags: ["monk-strap", "suede", "burgundy", "formal"],
  },

  // Accessories
  {
    id: 28,
    name: "Dhaka Gold Chronograph",
    category: "Accessories",
    price: 85000,
    rating: 4.9,
    reviews: 34,
    badge: "Editor's Pick",
    image:
      "https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=600&q=80",
    description:
      "A stunning gold-tone chronograph watch with a sunburst dial and sapphire crystal glass. Swiss movement, stainless steel case, and a genuine leather strap. A timepiece that speaks of success.",
    tags: ["watch", "chronograph", "gold", "luxury"],
  },
  {
    id: 29,
    name: "Bengal Leather Bifold Wallet",
    category: "Accessories",
    price: 6500,
    rating: 4.8,
    reviews: 156,
    badge: "Bestseller",
    image:
      "https://images.unsplash.com/photo-1627123424574-724758594785?w=600&q=80",
    description:
      "A slim bifold wallet in vegetable-tanned dark brown leather. Six card slots, two note compartments, and a coin pocket. Crafted by hand in Dhaka by master leather artisans.",
    tags: ["wallet", "leather", "brown", "slim"],
  },
  {
    id: 30,
    name: "Rajshahi Woven Leather Belt",
    category: "Accessories",
    price: 7800,
    rating: 4.7,
    reviews: 88,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1553062407-98eeb64c6a62?w=600&q=80",
    description:
      "A handwoven leather belt in cognac with a polished silver pin buckle. 35mm width, full-grain leather, and reinforced stitching. Available in sizes 32–44.",
    tags: ["belt", "leather", "cognac", "woven"],
  },
  {
    id: 31,
    name: "Silver Cufflinks Set",
    category: "Accessories",
    price: 4500,
    rating: 4.8,
    reviews: 67,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1611591437281-460bfbe1220a?w=600&q=80",
    description:
      "Polished sterling silver cufflinks with an engraved geometric pattern. Presented in a luxury RAJA Atelier gift box. The perfect finishing touch for a formal ensemble.",
    tags: ["cufflinks", "silver", "formal", "gift"],
  },
  {
    id: 32,
    name: "Dhaka Leather Card Holder",
    category: "Accessories",
    price: 3800,
    rating: 4.6,
    reviews: 124,
    badge: "New",
    image:
      "https://images.unsplash.com/photo-1606760227091-3dd870d97f1d?w=600&q=80",
    description:
      "An ultra-slim card holder in smooth black leather. Holds up to 6 cards and features a central slip pocket for folded notes. Minimalist elegance at its finest.",
    tags: ["card-holder", "leather", "black", "minimal"],
  },
  {
    id: 33,
    name: "Bronze Dress Watch",
    category: "Accessories",
    price: 55000,
    rating: 4.8,
    reviews: 41,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1614164185128-e4ec99c436d7?w=600&q=80",
    description:
      "An understated dress watch with a bronze case and a cream dial. Japanese quartz movement, sapphire-coated mineral glass, and a hand-stitched leather strap in dark tan.",
    tags: ["watch", "dress", "bronze", "minimal"],
  },

  // Ties & Pocket Squares
  {
    id: 34,
    name: "Rajshahi Silk Tie - Navy",
    category: "Ties & Pocket Squares",
    price: 4200,
    rating: 4.9,
    reviews: 78,
    badge: "Bestseller",
    image:
      "https://images.unsplash.com/photo-1589756823695-278bc923f962?w=600&q=80",
    description:
      "A classic seven-fold tie in navy Rajshahi silk. A subtle diagonal weave catches the light beautifully. Hand-rolled edges and an interlining-free construction for a superior knot.",
    tags: ["tie", "silk", "navy", "formal"],
  },
  {
    id: 35,
    name: "Bengal Paisley Pocket Square",
    category: "Ties & Pocket Squares",
    price: 2800,
    rating: 4.7,
    reviews: 93,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1601924994987-69e26d50dc26?w=600&q=80",
    description:
      "A hand-rolled silk pocket square with a traditional Bengali paisley pattern in gold and cream on a navy ground. The perfect accent for a suit or blazer.",
    tags: ["pocket-square", "silk", "paisley", "formal"],
  },
  {
    id: 36,
    name: "Muslin Bow Tie",
    category: "Ties & Pocket Squares",
    price: 3500,
    rating: 4.6,
    reviews: 45,
    badge: "New",
    image:
      "https://images.unsplash.com/photo-1598033129183-c4f50c736f10?w=600&q=80",
    description:
      "A self-tie bow tie in fine Dhaka muslin. Available in classic black and ivory white. Adjustable neck strap. A nod to Bengal's greatest textile tradition.",
    tags: ["bow-tie", "muslin", "formal", "black-tie"],
  },
  {
    id: 37,
    name: "Silk Tie Set - Burgundy",
    category: "Ties & Pocket Squares",
    price: 6800,
    rating: 4.8,
    reviews: 62,
    badge: "Popular",
    image:
      "https://images.unsplash.com/photo-1507679799987-c73779587ccf?w=600&q=80",
    description:
      "A matched set of a pure silk tie and pocket square in rich burgundy. The tie features a classic diagonal ribbed weave and the pocket square has hand-rolled edges. Presented in a gift box.",
    tags: ["tie", "pocket-square", "set", "burgundy"],
  },

  // Knitwear & Sweaters
  {
    id: 38,
    name: "Cashmere Crewneck - Camel",
    category: "Knitwear & Sweaters",
    price: 28000,
    rating: 4.9,
    reviews: 71,
    badge: "Bestseller",
    image:
      "https://images.unsplash.com/photo-1576566588028-4147f3842f27?w=600&q=80",
    description:
      "Grade-A Mongolian cashmere in warm camel. A classic crewneck with ribbed cuffs and hem. Impossibly soft, endlessly versatile — the cornerstone of a luxury winter wardrobe.",
    tags: ["cashmere", "crewneck", "camel", "winter"],
  },
  {
    id: 39,
    name: "Merino V-Neck Sweater",
    category: "Knitwear & Sweaters",
    price: 16500,
    rating: 4.7,
    reviews: 84,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1434389677669-e08b4cac3105?w=600&q=80",
    description:
      "A fine-gauge merino wool V-neck in deep charcoal. Lightweight enough to layer under a suit jacket, warm enough to wear alone. Ribbed collar, cuffs, and hem.",
    tags: ["merino", "v-neck", "charcoal", "layering"],
  },
  {
    id: 40,
    name: "Cable Knit Rollneck",
    category: "Knitwear & Sweaters",
    price: 19000,
    rating: 4.6,
    reviews: 53,
    badge: "New",
    image:
      "https://images.unsplash.com/photo-1516826957135-700dedea698c?w=600&q=80",
    description:
      "A substantial cable-knit rollneck in ecru. Chunky construction in a wool-alpaca blend. The ultimate cold-weather luxury piece — wear it with chinos or under an overcoat.",
    tags: ["cable-knit", "rollneck", "ecru", "winter"],
  },
  {
    id: 41,
    name: "Silk-Cotton Polo Sweater",
    category: "Knitwear & Sweaters",
    price: 14000,
    rating: 4.7,
    reviews: 66,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1559563458-527698bf5295?w=600&q=80",
    description:
      "A fine-knit polo sweater in a silk-cotton blend in midnight navy. Three-button placket and a ribbed collar. Light enough for the cooler Dhaka evenings.",
    tags: ["polo-sweater", "silk-cotton", "navy", "smart"],
  },

  // Casual Wear
  {
    id: 42,
    name: "Bengal Linen T-Shirt",
    category: "Casual Wear",
    price: 4500,
    rating: 4.6,
    reviews: 187,
    badge: "Popular",
    image:
      "https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?w=600&q=80",
    description:
      "A premium linen T-shirt in soft white. Crew neck, short sleeves, and a relaxed fit. Breathable and lightweight for the Bangladeshi climate. The foundation of a luxury casual wardrobe.",
    tags: ["t-shirt", "linen", "white", "casual"],
  },
  {
    id: 43,
    name: "Dhaka Selvedge Denim",
    category: "Casual Wear",
    price: 18000,
    rating: 4.8,
    reviews: 74,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1542272604-787c3835535d?w=600&q=80",
    description:
      "Japanese selvedge denim in deep indigo. Straight leg, mid-rise cut. Raw hem finish. Designed to develop a unique fade pattern unique to the wearer over time. A luxury denim experience.",
    tags: ["denim", "selvedge", "indigo", "casual"],
  },
  {
    id: 44,
    name: "Polo Shirt - Racing Green",
    category: "Casual Wear",
    price: 8500,
    rating: 4.7,
    reviews: 96,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1625910513591-fc75462cf5c3?w=600&q=80",
    description:
      "A classic polo shirt in rich racing green. Piqué cotton construction, three-button placket, and a ribbed collar and cuffs. A staple of refined casual dressing.",
    tags: ["polo", "cotton", "green", "casual"],
  },
  {
    id: 45,
    name: "Linen Shorts - Stone",
    category: "Casual Wear",
    price: 7200,
    rating: 4.5,
    reviews: 58,
    badge: "New",
    image:
      "https://images.unsplash.com/photo-1591195853828-11db59a44f43?w=600&q=80",
    description:
      "Tailored linen shorts in warm stone. Mid-thigh length, two side pockets, and a button-fly. The luxury approach to warm-weather dressing.",
    tags: ["shorts", "linen", "stone", "summer"],
  },
  {
    id: 46,
    name: "French Terry Sweatshirt",
    category: "Casual Wear",
    price: 9800,
    rating: 4.6,
    reviews: 82,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1556821840-3a63f15732ce?w=600&q=80",
    description:
      "A premium French terry sweatshirt in faded slate. Raglan sleeves, a ribbed hem and cuffs, and a garment-washed finish for a lived-in feel from day one.",
    tags: ["sweatshirt", "cotton", "slate", "casual"],
  },

  // Formal Wear
  {
    id: 47,
    name: "Midnight Tuxedo",
    category: "Formal Wear",
    price: 72000,
    rating: 5.0,
    reviews: 28,
    badge: "Editor's Pick",
    image:
      "https://images.unsplash.com/photo-1611898872015-0571a9e38375?w=600&q=80",
    description:
      "The ultimate evening statement. A midnight black tuxedo with silk peak lapels, a covered placket, and a single satin button. Paired with slim-cut matching trousers with a satin stripe. Perfection personified.",
    tags: ["tuxedo", "black", "formal", "black-tie"],
  },
  {
    id: 48,
    name: "Ivory Sherwani",
    category: "Formal Wear",
    price: 55000,
    rating: 4.9,
    reviews: 47,
    badge: "Bestseller",
    image:
      "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=600&q=80",
    description:
      "An exquisitely crafted ivory sherwani in fine brocade silk. Intricate gold thread embroidery on the collar and cuffs. Paired with matching churidar trousers. The pinnacle of South Asian formal dressing.",
    tags: ["sherwani", "ivory", "brocade", "wedding"],
  },
  {
    id: 49,
    name: "Royal Blue Achkan",
    category: "Formal Wear",
    price: 48000,
    rating: 4.8,
    reviews: 36,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?w=600&q=80",
    description:
      "A majestic achkan in royal blue raw silk. Mandarin collar, seven-button front, and subtle self-fabric piping. An elegant choice for formal occasions and celebrations.",
    tags: ["achkan", "silk", "blue", "formal"],
  },
  {
    id: 50,
    name: "Black Velvet Dinner Jacket",
    category: "Formal Wear",
    price: 38000,
    rating: 4.7,
    reviews: 31,
    badge: "Limited",
    image:
      "https://images.unsplash.com/photo-1560250097-0b93528c311a?w=600&q=80",
    description:
      "A sumptuous velvet dinner jacket in deep black. Shawl lapel, two-button front, and a single vent. Wear it with black dress trousers and a white tuxedo shirt for an unforgettable evening look.",
    tags: ["dinner-jacket", "velvet", "black", "formal"],
  },
  {
    id: 51,
    name: "Champagne Bandhgala Suit",
    category: "Formal Wear",
    price: 62000,
    rating: 4.9,
    reviews: 24,
    badge: "New",
    image:
      "https://images.unsplash.com/photo-1617127365659-c47fa864d8bc?w=600&q=80",
    description:
      "A contemporary bandhgala suit in champagne silk. A mandarin collar and a clean single-button closure give this a modern, architectural feel. Paired with slim matching trousers.",
    tags: ["bandhgala", "silk", "champagne", "wedding"],
  },
  {
    id: 52,
    name: "Charcoal Morning Suit",
    category: "Formal Wear",
    price: 68000,
    rating: 4.8,
    reviews: 19,
    badge: null,
    image:
      "https://images.unsplash.com/photo-1593030761757-71fae45fa0e7?w=600&q=80",
    description:
      "A traditional morning suit in charcoal with subtle grey striping. Features a single-button cutaway coat, matching waistcoat, and formal striped trousers. The height of daytime formal elegance.",
    tags: ["morning-suit", "charcoal", "formal", "traditional"],
  },
];

export const CATEGORIES = [
  "All",
  ...Array.from(new Set(PRODUCTS.map((p) => p.category))),
];
